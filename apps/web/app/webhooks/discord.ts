...
export async function publishSwapEvent(
  ...
  const { currentReserve, currentSupply, decimals, progress } =
    token.bondingCurve;
  ...
  const alertMessage =
    event.swapType === "Buy" ? "🤑 **NEW MINT** 🤑" : "🔥 **NEW BURN** 🔥";
...
export async function publishCreateAlert(
...
export async function publishHarvestAlert(
  event: HarvestEvent,
  token: TokenCard,
) {
