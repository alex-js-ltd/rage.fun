...
export function useSignAndSendTx(tx?: Uint8Array) {
  ...
  const { run, data, setData, setError, isLoading, ...rest } = useAsync<
    string | undefined | SignatureStatus
  >();
  ...
  useChannel("signatureEvent", (message: Ably.Message) => {
...
export function isSignatureStatus(data: unknown): data is SignatureStatus {
  return (
    ...
    (typeof (data as SignatureStatus).confirmations === "number" ||
      (data as SignatureStatus).confirmations === null) &&
