...
export function useAsync<DataType>() {
	const [{ data, error, status }, setState] = React.useReducer(
		(state: AsyncState<DataType>, action: AsyncAction<DataType>): AsyncState<DataType> => {
			switch (action.type) {
				case 'idle': {
					return {
						status: 'idle',
	...
	const setError = React.useCallback((error: unknown) => safeSetState({ error, type: 'rejected' }), [safeSetState])
	const reset = React.useCallback(() => safeSetState({ type: 'idle' }), [safeSetState])
