	...
	variants: {
		variant: {
			...
			interval: [
				...
				'disabled:bg-gray-100 disabled:text-gray-400 disabled:ring-0 has-[:focus-visible]:ring-2',
...
function Button({ className, variant, asChild = false, ref, ...props }: ButtonProps) {
	...
	return <Comp disabled={pending} className={cn(buttonVariants({ variant, className }))} ref={ref} {...props} />
}
