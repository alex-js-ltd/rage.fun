...
export function ConnectButton() {
  const { wallet, connected, publicKey, disconnect } = useUnifiedWallet();
  ...
  if (!connected || !publicKey) {
    return (
      <UnifiedWalletButton
        overrideContent={
          <button className="relative h-auto sm:rounded-full xl:flex items-center p-3 xl:w-41.25 w-full justify-center font-semibold text-text-200 xl:border xl:border-white xl:border-opacity-[0.125] hover:bg-white/10">
  ...
  return (
    <PopoverRoot>
      <PopoverTrigger className="sm:w-fit w-full">
        <div className="relative h-auto  xl:h-[65.55px] sm:rounded-full hover:bg-white/10 sm:w-fit flex items-center p-3 gap-3 justify-center">
          ...
          <Icon
            ...
            className="hidden xl:block size-6 text-text-200 "
          />
        </div>
      </PopoverTrigger>
      ...
      <PopoverPortal>
        <PopoverContent
          className="w-fit z-50 h-14 p-4 rounded-xl border border-white/0 shadow-lg outline-none bg-white/10"
          side="top"
          align="center"
          sideOffset={4}
        >
