...
import { ConnectButton } from "@/app/comps/connect_button";
import Image from "next/image";
import { NavLink, type NavLinkProps } from "@/app/comps/ui/nav_link";
...
function Desktop() {
  return (
    <nav className="hidden sm:block sticky top-0 ">
      <div className="h-13 flex items-center w-full ">
        <Link
          href={{
            pathname: "/home",
          }}
          className="xl:ml-0 ml-auto w-17.5 h-13 flex items-center justify-center cursor-pointer"
        >
          <Image
            className=""
      ...
      <div className="ml-auto xl:ml-0 w-17.5 xl:w-full items-center xl:items-start flex flex-col gap-1  h-[calc(100vh-52px)] pb-3">
        {NAV_ITEMS.map((l) => (
          <NavLink
            key={l.href.pathname}
            href={l.href}
          ...
          >
            {({ isActive }) => (
              <>
                <Icon
                  className={cn("size-6", isActive && "text-white")}
                  name={l.icon}
                />

                <span
                  className={cn(
                    "hidden xl:block  font-semibold text-lg",
                    isActive && "text-white",
                  )}
                >
                  {l.label}
