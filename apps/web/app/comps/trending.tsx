...
export function Trending({ trendingPromise }: { trendingPromise: Promise<Trending[]> }) {
	const data = use(trendingPromise)

	const [state, setState] = useState(data)

	useChannel('trendingEvent', (message: Ably.Message) => {
		const e: { tokens: Trending[] } = message.data

		setState(e.tokens)
	})

	return (
		<div className="border border-white/12.5 rounded-2xl w-full overflow-hidden">
...
function ListItem({ token }: { token: Trending }) {
