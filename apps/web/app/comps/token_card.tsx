			...
			<div className="relative flex flex-col gap-4 p-4">
				<div className="flex gap-4">
					<div className="flex flex-col items-center gap-1">
						...
						<span className="text-text-200 text-xs font-medium">{shortAddress(mint)}</span>
					</div>

					<div className="flex w-full flex-col gap-2">
						<div className="flex items-center gap-2">
							<span className="text-text-100 w-fit text-[16px] font-medium">{symbol}</span>
							<span className="text-white">|</span>
							<span className="text-text-200 w-fit text-[14px] font-medium">{name}</span>
						</div>
				...
				<div className="flex flex-wrap items-center gap-2 sm:h-8">
					...
					<div className="xxs:justify-end flex flex-1 items-center">{action}</div>
				</div>
			</div>
		</article>
	)
...
export function Pill({ label, value, tooltip, className }: PillProps) {
	const body = (
		<div className={`hover:bg-background-100 flex w-fit rounded-full px-2 py-1 ${className ?? ''}`}>
	...
	return (
		<Tooltip>
			...
			<TooltipContent side="bottom" sideOffset={4} className="bg-background-100 text-text-200 rounded-sm p-1 text-xs">
				{tooltip}
			</TooltipContent>
		</Tooltip>
	)
			...
			<div className="relative flex flex-col gap-4 p-4">
				...
				<div className="flex flex-wrap items-center gap-2 sm:h-8">
					<div className="flex flex-wrap items-center gap-1 rounded-full border border-white/5 px-1 py-1">
						<Loading className="relative h-4 w-15 shrink-0 cursor-pointer overflow-hidden rounded-md" i={4}></Loading>

						<Loading className="relative h-4 w-15 shrink-0 cursor-pointer overflow-hidden rounded-md" i={5}></Loading>

						<Loading className="relative h-4 w-15 shrink-0 cursor-pointer overflow-hidden rounded-md" i={6}></Loading>

						<Loading className="relative h-4 w-15 shrink-0 cursor-pointer overflow-hidden rounded-md" i={7}></Loading>
					</div>

					<div className="xxs:justify-end flex flex-1 items-center">{children ? children : null}</div>
				</div>
			</div>
		</article>
	)
