...
export default async function Page(props: Props) {
  ...
  const tokenPromise = getTokenFeed({
    sortType,
    sortOrder,
    cursorId,
    creatorId,
  ...
  return (
    <div className="flex-1 border-white/5 border-x ">
      <PageHeader />

      <div className="relative mx-auto flex max-w-150 flex-col pb-0 border-t border-white/5 min-h-[calc(100vh-52px)]">
          ...
          {isCreator ? (
            ...
            >
              <TokenFeed
                ...
                fallback={<TokenFeedFallback isEarnPage />}
              />
            </Suspense>
          ) : (
            <div className="p-4 flex flex-col gap-4">
              <p className="text-sm text-text-200">
                Launch a token on a{" "}
                <strong>decentralized bonding curve. </strong>
                Earn 1% from every trade, instantly — no middlemen, no
                extraction.
              </p>
            </div>
          )}
