...
export const getTopHolders = cache(
  async (address: string): Promise<TopHolder[]> => {
    ...
    const token0CreatorAta = await getAssociatedTokenAddress(
      mint,
    ...
    const { decimals, currentSupply } = await fetchBondingCurveState({
      program,
      mint,
    });
    ...
    const result = results.reduce<TopHolder[]>((acc, curr) => {
      ...
      const isCreator =
        curr.value.address.toBase58() === token0CreatorAta.toBase58();
...
function toTopHolder(decimals: number, totalSupply: BN) {
  return function (
    ...
    const percentageOwned = calculatePercentage(
      new BN(amount),
      totalSupply,
    ).toFixed(3);
